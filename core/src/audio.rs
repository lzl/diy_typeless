 ...
 static RECORDING_STATE: Lazy<Mutex<RecordingState>> = Lazy::new(|| Mutex::new(RecordingState::new()));
...
fn capture_u16(data: &[u16], channels: u16, samples: &Arc<Mutex<Vec<f32>>>) {
 ...
 fn wav_bytes_from_samples(samples: &[f32]) -> Result<Vec<u8>, CoreError> {
     ...
     {
         let mut writer =
             hound::WavWriter::new(&mut cursor, spec).map_err(|e| {
                 CoreError::AudioProcessing(e.to_string())
             })?;
