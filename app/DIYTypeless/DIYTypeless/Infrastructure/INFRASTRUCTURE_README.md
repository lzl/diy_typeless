...
| `Infrastructure/FFI/CoreFFIRuntimeBootstrap.swift` | Binds app UniFFI exports into `DIYTypelessCore.CoreFFIRuntime` handlers |

The FFI bridge is used by:
- Audio transcription/polishing/LLM processing (through `DIYTypelessCore`)
- Warmup and recording control runtime hooks
...
1. Keep direct app-side UniFFI calls inside `CoreFFIRuntimeBootstrap`
2. Implement behavior in `DIYTypelessCore` Data/UseCases and Repositories
3. Prefer `CoreFFIRuntime` handlers for cross-target testability
